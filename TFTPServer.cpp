...
TFTPServer::TFTPServer(unsigned int port ,unsigned int ip_mode )
{
	...
	if( (cxn_status = apr_sockaddr_info_get(&sa, "::1",ip_mode, port, 0, mem_pool))==APR_SUCCESS)
	{
		if( (cxn_status = apr_socket_create(&s,APR_INET, SOCK_STREAM, APR_PROTO_TCP, mem_pool))==APR_SUCCESS)
		{
			...
			//apr_socket_timeout_set(s, DEF_SOCK_TIMEOUT);//El 0 va con la condicion de no bloqueante. Hace que el intento de aceptar conexiones sea puntual.
